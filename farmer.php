<?php
session_start();
if (empty($_SESSION['user_id']) || ($_SESSION['user_role'] ?? '') !== 'farmer') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_product'])) {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $targetFile)) {
                $imagePath = 'uploads/products/' . basename($targetFile);
            }
        }

        $stmt = $conn->prepare("INSERT INTO products (farmer_id, title, description, image_path) VALUES (?, ?, ?, ?)");
        $stmt->bind_param('isss', $userId, $title, $description, $imagePath);
        if ($stmt->execute()) {
    ...
    if (isset($_POST['request_vehicle'])) {
        ...
        if ($driverId) {
            $stmt = $conn->prepare("INSERT INTO transport_requests (requester_id, requester_name, requester_role, product_id, vehicle_id, driver_id, details) VALUES (?, ?, 'farmer', ?, ?, ?, ?)");
            $stmt->bind_param('isisis', $userId, $requesterName, $productId, $vehicleId, $driverId, $details);
        } else {
            $stmt = $conn->prepare("INSERT INTO transport_requests (requester_id, requester_name, requester_role, product_id, vehicle_id, driver_id, details) VALUES (?, ?, 'farmer', ?, ?, NULL, ?)");
...
$products = $conn->query("SELECT * FROM products WHERE farmer_id = $userId ORDER BY created_at DESC");
...
$requests = $conn->query("SELECT tr.id, tr.status, tr.created_at, v.name AS vehicle_name, p.title AS product_title, d.first_name AS driver_first, d.middle_name AS driver_middle, d.last_name AS driver_last
    ...
    WHERE tr.requester_id = $userId AND tr.requester_role = 'farmer'
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .small-button { display: inline-block; margin-top: 12px; background: #e74c3c; }
        .small-button:hover { background: #c0392b; }
...
<div class="wrapper">
    ...
    <div class="card">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <input type="submit" name="add_product" value="Upload Product">
        </form>
    </div>
    ...
    <div class="card">
                    ...
                    <div class="product-card">
                        ...
                        <div class="content">
                            ...
                            <form method="POST">
                                ...
                                <button type="submit" name="delete_product" class="small-button">Delete</button>
                            </form>
                        </div>
                    </div>
                <?php endwhile; ?>
        ...
        <?php else: ?>
            <p class="no-items">No products uploaded yet.</p>
        <?php endif; ?>
    </div>
    ...
    <div class="card">
        ...
        <?php if ($availableVehicles && $availableVehicles->num_rows): ?>
            <form method="POST">
                <label for="product_id">Choose Product (Optional)</label>
                <select id="product_id" name="product_id">
                    <option value="">No specific product</option>
                    <?php
                    $ownProducts = $conn->query("SELECT id, title FROM products WHERE farmer_id = $userId ORDER BY created_at DESC");
                    while ($prod = $ownProducts->fetch_assoc()):
                    ?>
                ...
                <select id="vehicle_id" name="vehicle_id" required>
                    <?php 
                    $availableVehicles = $conn->query("SELECT * FROM vehicles WHERE status = 'available' ORDER BY created_at DESC");
                    while ($vehicle = $availableVehicles->fetch_assoc()): ?>
    ...
    <div class="card">
        ...
        <?php else: ?>
            <p class="no-items">No vehicle requests yet.</p>
        <?php endif; ?>
    </div>
</div>
</body>
